...
public class TransmutationTableBlock extends BlockWithEntity implements BlockEntityProvider, ImplementedInventory {
    ...
    public void saveData(World world, BlockEntity transmutationTableBlockEntity) {
        transmutationTableBlockEntity.markDirty();
        transmutationTableBlockEntity.toUpdatePacket();
        if (world != null && !world.isClient()) {
            world.updateListeners(transmutationTableBlockEntity.getPos(), transmutationTableBlockEntity.getCachedState(),
                    transmutationTableBlockEntity.getCachedState(), Block.NOTIFY_ALL);
        }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (!world.isClient) {
            // If you have research notes, attempt to craft
            if (player.getStackInHand(hand).getItem().equals(ModItems.WAND_OF_FROST_BOLT)) {
                TransmutationTableBlockEntity transmutationTableBlockEntity = (TransmutationTableBlockEntity) world.getBlockEntity(pos);
                assert transmutationTableBlockEntity != null;
                TransmutationTableBlockEntity.craftItem(transmutationTableBlockEntity, player);
                saveData(world, transmutationTableBlockEntity);
                world.getServer().sendMessage(Text.literal("attempting craft"));
                return ActionResult.SUCCESS;
            }

            // Grabs the inventory of the block entity at this blockstate's position.
            Inventory transmutationTableInventory = (Inventory) world.getBlockEntity(pos);

            // If the pedestal has an item, then give it back to the player, save the pedestal's data, and return.
            assert transmutationTableInventory != null;
            if (!transmutationTableInventory.getStack(0).isEmpty()) {

                player.getInventory().offerOrDrop(transmutationTableInventory.getStack(0));
                transmutationTableInventory.setStack(0, ItemStack.EMPTY);
            ...
            if (transmutationTableInventory.getStack(0).isEmpty() && !player.getStackInHand(hand).isEmpty()) {
                ...
                transmutationTableInventory.setStack(0, copyOfPlayerHeldItemStack);
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (state.getBlock() != newState.getBlock()) {
            ...
            if (thisBlockEntity instanceof TransmutationTableBlockEntity transmutationTableBlockEntity) {
                BlockPos itemScattererPos = thisBlockEntity.getPos().add(0,1,0);
                ItemScatterer.spawn(world, itemScattererPos, transmutationTableBlockEntity);
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (world.isClient)  {
            return checkType(type, ModBlockEntities.TRANSMUTATION_TABLE, (TransmutationTableBlockEntity::tick));
        } else {
            return checkType(type, ModBlockEntities.TRANSMUTATION_TABLE, (TransmutationTableBlockEntity::serverTick));
        }
    }
